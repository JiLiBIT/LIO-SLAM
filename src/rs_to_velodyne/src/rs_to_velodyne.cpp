...
int main(int argc, char **argv) {
    ...
    if (argc < 3) {
        ...
        if (std::strcmp("XYZI", argv[1]) == 0) {
            subRobosensePC = nh.subscribe("/rslidar2_points", 1, rsHandler_XYZI);
            // subRobosensePC = nh.subscribe("/rslidar_points_80", 1, rsHandler_XYZI);
            // subRobosensePC = nh.subscribe("/rslidar_points_80", 1, rsHandler_XYZI);
        } else if (std::strcmp("XYZIRT", argv[1]) == 0) {
            subRobosensePC = nh.subscribe("/rslidar2_points", 1, rsHandler_XYZIRT);
    ...
    ROS_INFO("Listening to /rslidar2_points ......");
