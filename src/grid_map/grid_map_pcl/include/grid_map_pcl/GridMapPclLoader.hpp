...
class GridMapPclLoader {
 ...
 public:
  ...
  /*!
   * Loads the point cloud into memory
   * @param[in] rosmessage to the point cloud.
   */
 ...
 protected:
  ...
  // liji
  /*!
   * interpolation the elevation at the linear index. The
   * algorithm is applied for each cell after the pre-processing has been done.
   * This function is designed such that it can be called in a parallel for loop.
   * There are no race conditions since each thread processed different
   * set of grid map cells.
   ...
   * @param[out] matrix of elevation values which need to be computed
   */
  void interpolationGridMapCell(const unsigned int linearGridMapIndex, grid_map::Matrix* tempMapData, grid_map::Matrix* gridMapData);
  ...
  std::vector<std::pair<int, int>> findNearestValidValues(const unsigned int linearGridMapIndex, grid_map::Matrix* gridMapData);

  std::pair<int, int> findNearestValidValue(const unsigned int linearGridMapIndex, grid_map::Matrix* gridMapData);

  void fillNaNGridMapCell(const unsigned int linearGridMapIndex, grid_map::Matrix* gridMapData);
  /*!
  ...
  float calculateElevationFromPointsInsideGridMapCellNoCluster(Pointcloud::ConstPtr cloud) const;
  /*!
