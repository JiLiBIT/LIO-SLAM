...
void processPointcloud(grid_map::GridMapPclLoader* gridMapPclLoader, const ros::NodeHandle& nh) {
  ...
  printTimeElapsedToRosInfoStream(start, "preProcessInputCloud took: ");
...
Eigen::Vector3d calculateMiniumMeanOfPointPositions(Pointcloud::ConstPtr inputCloud) {
  std::vector<Point> points(inputCloud->points.begin(), inputCloud->points.end());
  std::sort(points.begin(), points.end(), [](Point a, Point b) {
    return a.z < b.z;
  });
  const int numPointsToAverage = std::min(3, static_cast<int>(points.size()));
  points.erase(points.begin() + numPointsToAverage, points.end());

  Eigen::Vector3d mean = Eigen::Vector3d::Zero();
  for (const auto& point : points) {
    mean += Eigen::Vector3d(point.x, point.y, point.z);
  }
  mean /= numPointsToAverage;
...
float thisPoseYaw = 0.0;
float thisPosePitch = 0.0;
float thisPoseRoll = 0.0;
Pointcloud::Ptr filtedCloudPub;
} /* namespace grid_map_pcl*/
} /* namespace grid_map */
