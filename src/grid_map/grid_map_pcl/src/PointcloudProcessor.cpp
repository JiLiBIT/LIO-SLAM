...
Pointcloud::Ptr PointcloudProcessor::filtFromInputCloud(Pointcloud::ConstPtr inputCloud) const {
  Pointcloud::Ptr filteredCloud(new Pointcloud());
    for (const auto& point : *inputCloud) {
        if ( std::abs(point.x) < 20.0 && std::abs(point.y) < 30.0 ) { 
          if ( std::abs(point.x) < 2.5 && std::abs(point.y) < 5.0 ) {
            if (point.z < 1.0) {
...
Pointcloud::Ptr PointcloudProcessor::applyRigidBodyTransformation(Pointcloud::ConstPtr inputCloud) const {
  ...
  Eigen::Vector3d rpyIntrinsic(-thisPoseRoll,-thisPosePitch,0.0);
  auto transformedCloud =
      grid_map_pcl::transformCloud(inputCloud, grid_map_pcl::getRigidBodyTransform(translation,
                                                                                   rpyIntrinsic));

  return transformedCloud;
}

Pointcloud::Ptr PointcloudProcessor::applyReverseRigidBodyTransformation(Pointcloud::ConstPtr inputCloud) const {
  // auto transformedCloud =
  //     grid_map_pcl::transformCloud(inputCloud, grid_map_pcl::getRigidBodyTransform(params_->get().cloudTransformation_.translation_,
  //                                                                                  params_->get().cloudTransformation_.rpyIntrinsic_));
  Eigen::Vector3d translation(0.0,0.0,0.0);
