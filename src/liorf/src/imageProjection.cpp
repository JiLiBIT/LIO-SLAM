...
public:
    ...
    void cloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudMsg)
    {
        ...
        {
            cout << "cachePointCloud" << endl;
            return;
        }

        if (!deskewInfo())
        {
            cout << "deskewInfo" << endl;
            return;
        }
    ...
    void projectPointCloud()
    {
        ...
        for (int i = 0; i < cloudSize; ++i)
        {
            ...
            // if ((thisPoint.y < lidarMinFront && -lidarMinBack <  thisPoint.y  &&
            //      thisPoint.x < lidarMinLeft  && -lidarMinRight < thisPoint.x) ||
            //     range > lidarMaxRange)
            if ((thisPoint.y < lidarMinFront && -lidarMinBack <  thisPoint.y  &&
                thisPoint.x < lidarMinLeft  && -lidarMinRight < thisPoint.x) ||
            range > lidarMaxRange || thisPoint.intensity > lidarMaxIntensity)
                continue;
